...
**Limite conhecido.** Cópia ANTIGA ainda viva: o modo legacy protege configurações e cadastros, mas uma edição LEGÍTIMA feita nela em item que já existia (por exemplo o peso do perfil) é descartada; o que a barra de fato é a regra do servidor (item 11). Documento com ~534 KB (limite do Firestore 1 MiB): cresce ~100 KB/mês; sem mudança de modelo (dias antigos em subcoleção) a gravação começa a falhar por volta de fevereiro de 2027. Pendência estrutural registrada.
