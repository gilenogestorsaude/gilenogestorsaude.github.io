...
**Versão atual em produção:** v1.29.0 (PDF do treino com o extrato detalhado do mês + extrato em PDF; commitada 17/09, publicação com ok do CEO, ver parte 21) sobre a v1.28.2 (PDF gerado no aparelho + impressão por navegação real, 16/09 08h41) e a v1.28.0 (sincronização segura entre cópias: revisão + junção de 3 vias + escuta do documento, no ar 16/09 07h23), push do Mac mini pela deploy key com ok do CEO (ver parte 20). Antes: v1.27.0 (15/09 16h4x), v1.26.0 (14/09 18h08), v1.25.2 (14/09 14h07), v1.25.1 (14/09 09h07), v1.25.0 (14/09 08h07), v1.24.0 (02/09).
...
**Auditoria independente de 17/09 (subagente, harness próprio 52/52 no jsc, 551/551 no fitz com estrutura e regressão, 2/2 no motor real: Chrome headless gerou os PDFs byte-idênticos aos do jsc): APROVADO COM RESSALVAS, nenhum achado bloqueante ou alto.** Curados antes do commit: (A1) mês do acordo sem treino, as verticais da tabela cortavam "Nenhum treino neste mês" (agora param no cabeçalho); (A2) a duração entrava crua no HTML da lista (agora `escHtml`, defesa em profundidade: no app ela nasce numérica). Notas aceitas: o botão 📄 PDF do extrato só existe no visor interno (app instalado ou janela nova bloqueada), como o do registro desde a v1.28.2; `pdfTxt` descarta emoji e símbolos fora do WinAnsi (herdado); `pdfExtratoPages`/`personalMesListaHtml` leem o global `D`. Não verificado por ninguém daqui: folha de compartilhamento do iOS e Safari do iPhone (prova de campo pelo CEO), impressão real do `@media print` do extrato, segundo leitor de PDF além do MuPDF.
