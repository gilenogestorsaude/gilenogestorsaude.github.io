...
**Versão atual em produção:** v1.28.1 (impressão em tela cheia no app instalado) sobre a v1.28.0 (sincronização segura entre cópias: revisão + junção de 3 vias + escuta do documento, no ar 16/09 07h23), push do Mac mini pela deploy key com ok do CEO (ver parte 20). Antes: v1.27.0 (15/09 16h4x), v1.26.0 (14/09 18h08), v1.25.2 (14/09 14h07), v1.25.1 (14/09 09h07), v1.25.0 (14/09 08h07), v1.24.0 (02/09).
...
**Limite conhecido.** Cópia ANTIGA ainda viva: o modo legacy protege configurações e cadastros, mas uma edição LEGÍTIMA feita nela em item que já existia (por exemplo o peso do perfil) é descartada; o que a barra de fato é a regra do servidor (item 12). Documento com ~534 KB (limite do Firestore 1 MiB): cresce ~100 KB/mês; sem mudança de modelo (dias antigos em subcoleção) a gravação começa a falhar por volta de fevereiro de 2027. Pendência estrutural registrada.
